...
async function ensureRecurringEvents(team_id) {
  try {
    ...
    const { data: templates } = await supabase
      .from('events')
      .select('id, team_id, type, date, location, notes, match_location, visibility_type, group_id, coach_id, is_recurring, attendance(player_id, user_id, is_convoked)')
      .eq('team_id', team_id)
      ...
      while (scanner <= windowEnd) {
        ...
        if (template.attendance?.length > 0) {
          const convocations = template.attendance
            .filter(a => a.is_convoked && (a.player_id || a.user_id))
            .map(a => ({
              event_id: newEv.id,
          ...
          if (convocations.length > 0) {
            // Split into 2 batches: Player-based and User-based (Coaches/Members without player profile)
            const playerBatch = convocations.filter(c => c.player_id);
            const userBatch = convocations.filter(c => !c.player_id && c.user_id);

            if (playerBatch.length > 0) {
              await supabase.from('attendance').upsert(playerBatch, { onConflict: 'event_id, player_id' });
            }
            if (userBatch.length > 0) {
              await supabase.from('attendance').upsert(userBatch, { onConflict: 'event_id, user_id' });
            }
          }
        }
      }
...
router.get('/', requireAuth, async (req, res) => {
  try {
    const { team_id, maintenance } = req.query;
    if (!team_id || team_id === 'null') return res.json([]);

    // Trigger maintenance tasks ONLY if explicitly requested
    if (maintenance === 'true') {
      try {
    ...
    // 2. Build Query
    // [MODIFIED] Check range or specific month
